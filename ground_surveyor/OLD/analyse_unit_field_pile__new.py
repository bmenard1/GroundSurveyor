...
def load_pile(file_list_selected):
	...
	'n_pixel_at_zero_intensity': numpy.zeros(dc.n_file),
        'median_image_raw': numpy.zeros(n_pix_unit_field_on_the_side,n_pix_unit_field_on_the_side)
        'median_image_small': numpy.zeros(n_pix_unit_field_on_the_side,n_pix_unit_field_on_the_side)
        'median_image_large': numpy.zeros(n_pix_unit_field_on_the_side,n_pix_unit_field_on_the_side)
	}
...
### here we expect to ingest a pile of unit fields

    	unit_field_pile = gdal_array.LoadFile(unit_field_filename_path)
        unit_field_metadata_filename_path = os.path.splitext(unit_field_filename_path)[0] + '.json'


## here we are still reading a json file per field. Not good.
    ...
    ## indexing will depend on Frank's convention
...
def compute_median(dc):
	...
	for j_pix in range(n_pix_unit_field_on_the_side):
            ...
            dc.metadata['median_image_raw'[i_pix,j_pix] = \
                numpy.median( dc.cube[0,:,i_pix,j_pix])
            dc.metadata['median_image_small'[i_pix,j_pix] = \
                numpy.median( dc.cube[1,:,i_pix,j_pix])
            dc.metadata['median_image_large'[i_pix,j_pix] = \
